...
class TimeFlags(object):
    ...
    def filter_steps(self):
        ...
        if self.command.flag("after"):
            after_epoch_secs = (
                datetime.datetime.strptime(self.command.flag("after"), "%Y-%m-%d").astimezone(tz.tzlocal()).timestamp()
            )
            steps.append({"$match": {"gameCreation": {"$gt": 1000 * after_epoch_secs}}})
        if self.command.flag("before"):
            before_epoch_secs = (
                datetime.datetime.strptime(self.command.flag("before"), "%Y-%m-%d").astimezone(tz.tzlocal()).timestamp()
            )
